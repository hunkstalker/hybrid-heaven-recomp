...
Fase actual: **gameplay** (menús → GAME START → escenas 3D y combate) con mando Xbox, audio a 43200 Hz y Controller Pak emulado. **Guardado en cápsula: VALIDADO en Windows (2026-09-16)**. Pacing/cadencia: **CERRADO** (2026-09-17; logs de diagnóstico ahora opt-in con `HH_DIAG=1`). **BLOQUEANTE**: freeze al entrar en **combate cuerpo a cuerpo** (`[BADMQ]` + objeto `0x8024A990` con callback `0xFFFF84CD`). **Diferencial port↔emulador con el mismo input HECHO** y **CaC reproducido y acotado con el replay de BizHawk (2026-09-17)**: el mantenedor grabó una partida en BizHawk (`bizhawk_hh_tracker_v3.lua`) y, convertida (`bizhawk_to_replay.py --vis-offset -100`), **el port se cuelga igual** (`[BADMQ]`, watchdog VI=32122) mientras el emulador no. **La ráfaga #22 SÍ se ejecuta** → la teoría del "burst saltado" queda **refutada**; el primer desvío nuevo es la **carga #49 (~VI 20500)**. Siguiente: dumps finos 20400-20800 en ambos lados y comparar la línea temporal del módulo 24/directorio. **El test de juego del port nativo lo hace el mantenedor (usuario)**; dev solo pasadas headless. Detalle: `notes/2026-09-17-bizhawk-replay-freeze-con-rafaga.md` y `notes/2026-09-17-cac-timeline-modulo24-periodo.md` §4b-5. El runtime tiene cambios **locales** sin push (usar `port\build_windows.local.bat`). Detalle y siguiente paso exacto: `RETOMAR.md` y `TODO.md`.
