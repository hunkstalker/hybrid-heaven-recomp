...
Fase actual: **gameplay** (menús → GAME START → escenas 3D y combate) con mando Xbox, audio a 43200 Hz y Controller Pak emulado. **Guardado en cápsula: VALIDADO en Windows (2026-09-16)**. Pacing/cadencia: **CERRADO** (2026-09-17; logs de diagnóstico ahora opt-in con `HH_DIAG=1`). **BLOQUEANTE**: freeze al entrar en **combate cuerpo a cuerpo** (`[BADMQ]` + objeto `0x8024A990` con callback `0xFFFF84CD`). **Diferencial port↔emulador con el mismo input HECHO** y **mecanismo y causa raíz de la primera divergencia (carga #22) localizados (2026-09-17)**: la ráfaga son pasos de guion de la **línea temporal del módulo 24**; el cambio de escena se adelanta porque `[0x80089478]&0x1000` (**START recién pulsado**) se pone durante la transición en el port (VI 765-798) y no en el emulador (nunca en la transición). Es un problema de **alineación del replay**: la grabación está indexada por frame con `vis` (~2.65 VI/frame) y el port aplica por frame con relación frame↔VI ~1.86, así que el START de menú (samples 412-431, `vis` 1092-1135) cae en la transición. Base de tiempo general refutada; **WIP (a medias)**: `HH_REPLAY_CLOCK=1` implementado (reloj esclavo + interpolación) pero el replay aún no es fiel (transición adelantada) → no cerrar conclusiones del CaC; **el test de juego del port nativo lo hace el mantenedor (usuario)**. Detalle: `notes/2026-09-17-cac-timeline-modulo24-periodo.md` §4b-5 y `notes/2026-09-17-replay-clock-y-desfase-frames-wip.md`. El runtime tiene cambios **locales** sin push (usar `port\build_windows.local.bat`). Detalle y siguiente paso exacto: `RETOMAR.md` y `TODO.md`.
