> **ARCHIVO HISTÓRICO — NO es fuente de verdad operativa.**
> `PROYECTO.md` versión pre-consolidación (2026-09-11). Se conserva íntegro como evidencia.
> La versión viva y recortada es `/PROYECTO.md`. No editar.
...
4. ~~**ARANCAR FASE 2 — ELF del código principal plano**~~ ✅ **HECHO**: ELF + build Linux/Windows +
   boot (`Entrypoint returned`, threads del juego, game loop corriendo) con la syms Ghidra. Audio
   dummy primero. **La Fase 2 ya no es la acción pendiente**; ver §9.1 y `sesion.md` §16.7 para el
   bloqueante actual. ← **PRÓXIMA ACCIÓN: resolver el clobber del `a0` (§9.1 / sesion.md §16.7 #16).**
5. ~~Construir el **mapa de símbolos inicial** del núcleo plano~~ ✅ **HECHO (vía Ghidra, mejor que por
   strings)**: `config/us_ghidra.syms.toml` (867 funcs, 94 os funcs de n64sym) + `ignored` de blobs
   de datos → regeneración limpia y game loop corriendo. Refinamiento futuro: filtrar datos-como-funciones,
   fusionar divisiones y añadir declaraciones faltantes a `funcs.h` (`notes/2026-09-10-ghidra-syms-solucion-fondo.md` §5).
...
8. **Variantes LZSS del cargador `trans`** (`LZSS 5`/`LZSS 7`; §4.6) — **EN CURSO, es el bloqueante
   actual** (ya no por el decode: el fix DMA carga el asset). El crash es el **clobber del `a0`** en el
   reload `FUN_80003D3C` vía la vía de runtime del DMA/threading (§9.1 + `sesion.md` §16.7 #16). El asset
   NO es LZKN64 plano y `lzkn64` NO sirve de oráculo (`notes/2026-09-10-lzss-decompressor-crash.md`
   §6.1-6.4); oráculo válido = binario descomprimido real en RAM (tarea #3 / BizHawk).
...
### 9.1 FASE 2 (boot/recompilación) — estado actual 2026-09-11 (para retomar)
...
- **BLOQUEANTE ACTUAL — clobber del `a0`**: el reload (`FUN_80003D3C`) guarda `a0=0x80089518` en
  ...
  **CLUE §6.12**: el clobber ocurre **DENTRO de `FUN_80003db4`** (cadena DMA), durante **5 swaps
  thread 5↔RSP 16-19** (park/unpark); el watchpoint cae en pthread, NO en un write del recompilado.
  **Enfoques propuestos**: (a) instrumentar `wait_for_resumed`/`resume_thread_and_wait`/
  `run_next_thread_and_wait` (scheduling.cpp) para ver qué escribe `0x8005BE18` durante el park;
  (b) verificar qué es `0x8005BE18` (¿struct/cola compartida? ¿solapamiento de stacks entre threads?);
  (c) parchear el juego para DMA síncrona vía `do_rom_read` (estilo GoldenRecomp
