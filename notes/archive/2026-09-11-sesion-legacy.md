...
> `sesion.md` congelado el 2026-09-11 durante la consolidación de documentación.
> El estado y las tareas actuales están en `PROYECTO.md`, `TODO.md` y `docs/`.
> Se conserva íntegro como evidencia. No editar.
...
[✓] FIX CONCURRENCIA (2026-09-10, runtime): osStartThread desde el hilo de boot (thread_self==NULL) señalaba threads SIN aparcar el hilo de boot → game-threads corrían concurrentemente (violación single-CPU N64) → corrompían datos compartidos (heap/colas). FIX: lock global (acquire/release_game_lock) en ultramodern threads.cpp/mesgqueue.cpp + boot entrypoint (recomp.cpp). Correcto; NO arregla el crash del descompresor (determinista). Detalle: notes/2026-09-10-lzss-decompressor-crash.md §3
[ ] VALIDAR FIX DE CONCURRENCIA EN WINDOWS (independiente, NO bloquea el clobber del a0): compilar en Windows (MSVC, cmake --build build --target HybridHeavenRecomp) y comprobar que el lock single-CPU no introduce deadlocks/crashes NUEVOS más allá del crash del descompresor (ya esperado). El usuario pega boot.log/hh.log. (Commit del fix: submodule 56182ef.)
