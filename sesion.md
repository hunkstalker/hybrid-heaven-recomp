...
[•] BLOQUEANTE: RENDER (pantalla negra). DIAGNÓSTICO (2026-09-10): el game loop (thread 5) está clavado en osRecvMesg(0x8005be40) y NUNCA llega a su bucle de render (osRecvMesg mq=0x8005c288=0). DAT_80037750 (modo render)=0 es consecuencia. CAUSA RAÍZ: el motor Konami de HH usa un MECANISMO DE TAREAS CUSTOM (FUN_80029fa0 escribe tarea en 0x800cc300 + señal cop0 Status vía FUN_800326d0; dispatch escribe SP_STATUS 0xa4040010 directo) en vez de osSpTaskLoad/osSpTaskStartGo (que usa Goemon → el runtime lo intercepta). El runtime no intercepta el camino custom → la primera tarea nunca se envía al RSP → su finalización nunca postea a 0x8005be40 → game loop colgado. Detalle: notes/2026-09-10-render-blocker.md
[ ] FIX RENDER (decisión A, la técnicamente correcta — sin atajos): mapear a fondo el mecanismo custom (struct 0x800cc300, señal cop0 Status, dispatch SP) y hacer que el runtime lo soporte llegando a submit_rsp_task. NO disparar 0x8005be40 a mano (chapuza: mentiría al juego sobre una tarea no procesada)
