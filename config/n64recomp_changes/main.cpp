...
// Recompile a function into the output file. If the recompiler stubs the function out
// mid-way (e.g. a data block absorbed by a coarse symbol boundary produces an unbalanced
// body with dangling `if {` blocks), discard the buffered body and emit a clean trap stub.
bool recompile_function_clean(const N64Recomp::Context& context, size_t func_index, std::ostream& output_file, std::span<std::vector<uint32_t>> static_funcs_out) {
    std::ostringstream buffer;
    bool stubbed = false;
    const N64Recomp::Function& func = context.functions[func_index];
    if (!N64Recomp::recompile_function(context, func_index, buffer, static_funcs_out, false, &stubbed)) {
        return false;
    }
    if (stubbed) {
...
bool recompile_single_function(const N64Recomp::Context& context, size_t func_index, const std::string& recomp_include, const std::filesystem::path& output_path, std::span<std::vector<uint32_t>> static_funcs_out) {
    ...
    if (!recompile_function_clean(context, func_index, output_file, static_funcs_out)) {
        return false;
    }
...
int main(int argc, char** argv) {
    ...
    if (!config.elf_path.empty()) {
        ...
        // HH: en ELF mode tambien hay que propagar el flag (antes solo se hacia en la rama de
        // symbols-file); sin esto, las llamadas dentro de la misma seccion se emiten DIRECTAS y
        // saltan el lookup (y por tanto los hooks de loader registrados por direccion).
    ...
    for (size_t i = 0; i < context.functions.size(); i++) {
        ...
        if (!func.ignored && func.words.size() != 0) {
            ...
            if (config.single_file_output || config.functions_per_output_file > 1) {
                result = recompile_function_clean(context, i, current_output_file, static_funcs_by_section);
                if (!config.single_file_output) {
    ...
    for (size_t section_index = 0; section_index < context.sections.size(); section_index++) {
        ...
        for (size_t static_func_index = 0; static_func_index < section_statics.size(); static_func_index++) {
            ...
            if (config.single_file_output || config.functions_per_output_file > 1) {
                result = recompile_function_clean(context, new_func_index, current_output_file, static_funcs_by_section);
                if (!config.single_file_output) {
                    cur_file_function_count++;
                    if (cur_file_function_count >= config.functions_per_output_file) {
                        open_new_output_file();
