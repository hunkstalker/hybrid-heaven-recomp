...
bool process_instruction(GeneratorType& generator, const N64Recomp::Context& context, const N64Recomp::Function& func, size_t func_index, const N64Recomp::FunctionStats& stats, const std::unordered_set<uint32_t>& jtbl_lw_instructions, size_t instr_index, const std::vector<rabbitizer::InstructionCpu>& instructions, std::ostream& output_file, bool indent, bool emit_link_branch, int link_branch_index, size_t reloc_index, bool& needs_link_branch, bool& is_branch_likely, bool tag_reference_relocs, std::span<std::vector<uint32_t>> static_funcs_out) {
            ...
            if (reloc_reference_symbol != (size_t)-1) {
                ...
                switch (jal_result) {
                    ...
                    case JalResolutionResult::Match:
                        // Reimplemented/ignored functions have no generated body, so
                        // route them through the runtime lookup instead of a direct call.
                        if (context.functions[matched_func_index].reimplemented ||
                            context.functions[matched_func_index].ignored) {
                            call_by_lookup = true;
                        }
                        else {
    ...
    auto print_branch = [&](uint32_t branch_target) {
        ...
        if (branch_target < func.vram || branch_target >= func_vram_end) {
            ...
            // Unknown target (e.g. an overlay address not in the symbol list).
            // Compile as an indirect tail call, resolved at runtime via lookup.
    ...
    case InstrId::cpu_nop:
        fmt::print(output_file, "\n");
        break;
    case InstrId::cpu_INVALID:
        fmt::print(stderr, "[Warn] INVALID instruction treated as nop in {}\n", func.name);
        fmt::print(output_file, "\n");
        break;
    // COP2 (RSP) is not emulated; unusual in main code, mostly appears in data.
    case InstrId::cpu_lwc2:
    case InstrId::cpu_ldc2:
    case InstrId::cpu_swc2:
    case InstrId::cpu_sdc2:
    case InstrId::cpu_mfc2:
    case InstrId::cpu_mtc2:
    case InstrId::cpu_cfc2:
    case InstrId::cpu_ctc2:
        fmt::print(output_file, "\n");
        break;
    // Conditional moves are only present in a handful of overlay code paths;
    // treat them as no-ops to allow generation to complete.
    case InstrId::cpu_movz:
    case InstrId::cpu_movn:
        fmt::print(stderr, "[Warn] movz/movn treated as nop in {}\n", func.name);
        fmt::print(output_file, "\n");
        break;
    // Cache operations are cache-management hints; the host cache is not
    // managed by the recompiled code, so treat them as no-ops.
    case InstrId::cpu_cache:
    case InstrId::cpu_sync:
    case InstrId::cpu_pref:
        fmt::print(output_file, "\n");
        break;
    // TLB is not emulated in the flat recompile; the KSEG0/KSEG1 addresses used
    // by the game map 1:1 to physical. Treat TLB maintenance ops as no-ops.
    ...
    case InstrId::cpu_eret:
        fmt::print(stderr, "[Warn] eret treated as nop in {}\n", func.name);
        fmt::print(output_file, "\n");
        break;
    // Trap instructions raise an exception when a condition is met; they are
    // unused by the game's code paths outside of data blobs read as code, so
    // treat them as no-ops.
    case InstrId::cpu_teq:
    case InstrId::cpu_tge:
    case InstrId::cpu_tgeu:
    case InstrId::cpu_tlt:
    case InstrId::cpu_tltu:
    case InstrId::cpu_tne:
    case InstrId::cpu_teqi:
    case InstrId::cpu_tgei:
    case InstrId::cpu_tgeiu:
    case InstrId::cpu_tlti:
    case InstrId::cpu_tltiu:
    case InstrId::cpu_tnei:
        fmt::print(output_file, "\n");
        break;
    ...
    // Cop0 control register moves are not emulated; treat as no-ops.
    case InstrId::cpu_cfc0:
    case InstrId::cpu_ctc0:
        fmt::print(output_file, "\n");
        break;
    // Arithmetic
    ...
    case InstrId::cpu_jalr:
        ...
        if (rd != (int)rabbitizer::Registers::Cpu::GprO32::GPR_O32_ra) {
            fmt::print(stderr, "[Warn] jalr with non-ra return reg treated as nop in {}\n", func.name);
            break;
        }
    ...
    case InstrId::cpu_b:
        {
            ...
            // FIXME: how to deal with static functions?
                ...
                // Unknown target (e.g. an overlay address not in the symbol list).
                // Compile as an indirect tail call, resolved at runtime via lookup,
                // mirroring how jals to overlay functions are handled.
                if (!process_delay_slot(false)) {
                    return false;
                }
                print_indent();
...
bool recompile_function_impl(GeneratorType& generator, const N64Recomp::Context& context, size_t func_index, std::ostream& output_file, std::span<std::vector<uint32_t>> static_funcs_out, bool tag_reference_relocs) {
    ...
    if (!func.stubbed) {
        ...
        // A link branch (e.g. a jal whose delay slot lies outside this function) that
        // still has an unresolved `after_N` jump means the caller's continuation flows
        // into a neighboring function. This is usually a bad function split (e.g. in
        // overlay code). Emit the pending label so the generated C stays valid rather
        // than failing the whole generation.
        if (needs_link_branch) {
            fmt::print(stderr, "[Warn] Unhandled link branch at end of {}, emitting label\n", func.name);
            fmt::print(output_file, "    ");
