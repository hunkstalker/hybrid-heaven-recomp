...
| 3. Render (RT64) | en curso | RT64 **procesa DLs** (wiring OK). Arranque corregido en dos tandas: (1) 4 MB de RDRAM, (2) **des-stubbing de la init de libultra** (ADR 0002: `osInitialize`, PI manager, VI, `__osEventStateTab`…). El port **no aborta** y su cadena de boot coincide con el emulador. Sigue en `fase=0`: **loader y directorio Nisitenma descartados** (idénticos al emulador; el port sí carga idx54 vía `FUN_801079B0`). **Sentinel de colas libultra resuelto** (`osCreateMesgQueue`, `0x80049930`). Bloqueo actual: el **`OSViContext` del juego** (`0x8004AE70…`) nunca se inicializa/mantiene (`__osViInit`/`__osViSwapContext`=0 llamadas) ⇒ `FUN_80035050`=0 ⇒ el hilo 17 espera un cambio de framebuffer ⇒ `[0x8005CD4C]` queda en 2 y `FUN_80001454` no llama a `FUN_80005270`. Ver `notes/2026-09-13-vi-context-y-sentinel.md` |
